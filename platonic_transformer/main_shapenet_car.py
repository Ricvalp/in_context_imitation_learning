...
class PressureModel(pl.LightningModule):
    ...
    def forward(self, graph):
        ...
        # Forward pass
        pred_scalar, _ = self.net(x, pos, batch, vec=vec, avg_num_nodes=self.avg_num_nodes)
        ...
        if self.all_test_batches and self.logger and WANDB_AVAILABLE:
            ...
            for i, batch in enumerate(self.all_test_batches):
                ...
                for sample_idx_in_batch in range(num_samples_in_batch):
                    ...
                    sample_indices = torch.where(batch['batch_idx'] == sample_idx_in_batch)[0]
                    ...
                    pred_pressures = pred_pressures * self.dataset_std + self.dataset_mean
                    ...
                    self.logger.experiment.log({
                        "test_visualizations/Ground_Truth": self._create_point_cloud_log(points, gt_pressures, vmin, vmax),
                        "test_visualizations/Prediction": self._create_point_cloud_log(points, pred_pressures, vmin, vmax)
                    })
    ...
    def configure_optimizers(self):
        ...
        optim_groups = [
            {"params": [param_dict[p_name] for p_name in sorted(list(decay))], "weight_decay": self.hparams.weight_decay},
            {"params": [param_dict[p_name] for p_name in sorted(list(no_decay))], "weight_decay": 0.0},
        ]
        ...
        if self.hparams.cosine_scheduler:
            ...
            return {"optimizer": optimizer, "lr_scheduler": scheduler, "monitor": "valid_mse"}
        else:
            return {"optimizer": optimizer, "monitor": "valid_mse"}

def load_data(args):
    """Load and preprocess ShapenetCar dataset."""
    train_dataset = ShapenetCarDataset(data_path=args.data_dir, split='train', knn=args.knn)
    # Use the test set for both validation and testing
    test_dataset = ShapenetCarDataset(data_path=args.data_dir, split='test', knn=args.knn)
...
def main(args):
    ...
    callbacks = [
        pl.callbacks.ModelCheckpoint(monitor='valid_mse', mode='min', 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Shapenet-Car Pressure Prediction Training')
    ...
    parser.add_argument('--equivariance', type=str, default="SEn", help='Type of equivariance (SEn or Tn)')
    ...
    parser.add_argument('--freq_sigma', type=float, default=2.0, help='Sigma for RFF positional encoding.')
    parser.add_argument('--dropout', type=float, default=0.1, help='Dropout rate.')
    parser.add_argument('--norm_first', type=eval, default=True, help='Use LayerNorm before attention.')
    parser.add_argument('--learned_freqs', type=eval, default=True, help='Learn frequencies for RFF.')
    parser.add_argument('--dense_mode', type=eval, default=True, help='Use dense mode for attention.')
    parser.add_argument('--mean_aggregation', type=eval, default=False, help='Use mean aggregation in attention.')
    ...
    # Input features
    parser.add_argument('--scalar_features', type=eval, default=[], help='Features to use as scalars (e.g., ["coords"])')
    parser.add_argument('--vector_features', type=eval, default=["coords"], help='Features to use as vectors (e.g., ["coords"])')
    
    # Data and logging
    parser.add_argument('--data_dir', type=str, default='./datasets/shapenet_car/preprocessed', help='Data directory for ShapenetCar')
    parser.add_argument('--knn', type=int, default=16, help='k for k-NN graph construction in the dataset loader')
    parser.add_argument('--log', type=eval, default=True, help='Enable logging with wandb')

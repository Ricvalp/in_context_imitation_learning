...
def to_dense_and_mask(
    x: torch.Tensor, vec: torch.Tensor, pos: torch.Tensor, batch: Optional[torch.Tensor]
):
    """
    ...
    if (
        batch is None
    ):  # input was already dense, only create mask (TODO: What if is just set to None?)
        if not (
            x.ndim == 3
            and pos.ndim == 3
            and x.shape[0] == pos.shape[0]
            and x.shape[1] == pos.shape[1]
        ):
            raise ValueError(
                "If batch is None, x and pos must be [B, N, Dim] with matching B and N."
            )
        B, N, _ = x.shape
        ...
        attention_mask = torch.zeros(
            num_graphs, max_nodes_per_graph, dtype=torch.bool, device=pos.device
        )
...
def pool(
    ...
    if dense_mode:
        ...
        x = scatter_add(x, batch, dim_size=batch.max() + 1) / (
            avg_num_nodes if avg_num_nodes is not None else 1.0
        )
        # Normalize by number of nodes in each graph
        if mean_aggregation:
            num_nodes = scatter_add(
                torch.ones_like(batch, dtype=x.dtype), batch, dim_size=batch.max() + 1
            ).unsqueeze(-1)
        else:
            num_nodes = avg_num_nodes if avg_num_nodes is not None else 1.0
        x = x / num_nodes

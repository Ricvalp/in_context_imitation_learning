...
class QM9Model(pl.LightningModule):
    ...
    def forward(self, graph):
        ...
        positions_mean = scatter_add(graph.pos, graph.batch, dim_size=graph.batch.max().item() + 1) / scatter_add(torch.ones_like(graph.pos[:, :1]), graph.batch, dim_size=graph.batch.max().item() + 1)
    ...
    def set_dataset_statistics(self, dataloader):
        ...
        """
        stats_file = os.path.join(self.hparams.data_dir, f"stats_{self.hparams.target}.npz")
...
def main(args): #
    ...
    callbacks.append(StopOnPersistentDivergence(monitor="valid MAE", threshold=1.0, patience=10, grace_epochs=20, verbose=False))
    ...
    if args.test_ckpt is None:
        ...
        model = QM9Model.load_from_checkpoint(args.test_ckpt, hparams_file=os.path.join(os.path.dirname(args.test_ckpt), "hparams.yaml"), args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=1000, help='Number of training epochs.') # Adjusted default
    parser.add_argument('--timer', type=str, default=None, help='Timer for training, e.g., "00:08:00:00".')
    parser.add_argument('--warmup', type=int, default=10, help='Number of warmup epochs for cosine scheduler.')
    parser.add_argument('--batch_size', type=int, default=96, help='Batch size.')
    parser.add_argument('--lr', type=float, default=5e-4, help='Learning rate.')
    parser.add_argument('--weight_decay', type=float, default=1e-8, help='Weight decay.')
    parser.add_argument('--seed', type=int, default=42, help='Random seed.') # Changed default
    ...
    parser.add_argument('--hidden_dim', type=int, default=1152, help='Hidden dimension for the model.')
    ...
    parser.add_argument('--head_dim', type=int, default=None, help='Implicitly defines number of heads (Transformer only).')
    parser.add_argument('--rope_sigma', type=eval, default=1.0, help='Sigma for RFF positional encoding (Transformer only).')  # set to 4 if using spiral init
    parser.add_argument('--use_key', type=eval, default=False, help='Use key projection when using RoPE (Transformer only).')
    parser.add_argument('--ape_sigma', type=eval, default=0.5, help='Sigma for RFF positional encoding (Transformer only).')
    parser.add_argument('--freq_init', type=str, default='random', choices=['random', 'spiral'], help='Frequency initialization method for RoPE (Transformer only).')  # combo spiral-rope_sigma=4 or random-rope_sigma=1
    ...
    parser.add_argument('--solid_name', type=str, default="octahedron", help='tetrahedron, trivial_3, octahedron, icosahedron')
    ...
    parser.add_argument('--drop_path_rate', type=float, default=0.0, help='Stochastic depth rate (uniform).')

    # Training features
    parser.add_argument('--train_augm', type=eval, default=True, help='Use rotation augmentation during training.') #
    parser.add_argument('--use_k_hot_encoding', type=eval, default=True, help='Use k-hot encoding for atom types instead of one-hot.') #
    # Data and logging
    parser.add_argument('--data_dir', type=str, default="./datasets/qm9", help='Directory for QM9 dataset.')
    parser.add_argument('--target', type=str, default="mu", help='Target molecular property to predict from QM9.')
    parser.add_argument('--log', type=eval, default=True, help='Enable logging (e.g., to WandB).')
    parser.add_argument('--wandb_identity', type=str, default='m-m-islam-university-of-amsterdam', help='use <m-m-islam-university-of-amsterdam> for teamspace, or your own identity')
    ...
    parser.add_argument('--model_id', type=int, default=None, help='Model ID for labeling configurations.')

...
class CIFAR10Model(pl.LightningModule):
    ...
    def forward(self, data):
        ...
        pred, _ = self.net(
            data.x, data.pos, data.batch, vec=vec, avg_num_nodes=self.avg_num_nodes
        )
        return pred
...
def load_data(args):
    ...
    test_dataset = torchvision.datasets.CIFAR10(
        root=args.data_dir, train=False, transform=transform_test, download=True
    )
...
def main(args):
    ...
    if args.log:
        ...
        logger = pl.loggers.WandbLogger(
            project="Platonic-CIFAR10", config=args, save_dir=save_dir
        )
    else:
        logger = None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CIFAR10 Point Cloud Classification Training"
    )
    ...
    parser.add_argument(
        "--layers", type=eval, default=12, help="Number of layers or layers per scale"
    )
    parser.add_argument(
        "--solid_name",
        type=str,
    ...
    parser.add_argument(
        "--use_key", type=eval, default=False, help="Use key projection when using RoPE"
    )
    parser.add_argument(
        "--freq_init",
        type=str,
        ...
        help="Frequency initialization method for RoPE",
    )
    parser.add_argument(
        "--ape_sigma", type=eval, default=16.0, help="Sigma for RFF positional encoding"
    )
    ...
    parser.add_argument(
        "--ffn_readout", type=eval, default=False, help="Use FFN readout after pooling"
    )

...
class MDModel(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        in_channels_scalar = (
            ...
            * ("coords" in self.hparams.scalar_features)  # x,y,z coordinates as scalars
            + 3
            * ("velocity" in self.hparams.scalar_features)  # x,y,z velocity as scalars
        )
        in_channels_vector = 1 * (
            "coords" in self.hparams.vector_features
        ) + 1 * (  # position as vector
            "velocity" in self.hparams.vector_features
        )  # velocity as vector
    ...
    def set_dataset_statistics(self, train_loader):
        self.avg_num_nodes = sum(data.num_nodes for data in train_loader) / len(
    ...
    def forward(self, graph):
        ...
        x = (
            torch.ones(graph.pos.size(0), 1).type_as(graph.pos)
        ...
        pred_scalar, pred_vec = self.net(
            x, graph.pos, graph.batch, vec=vec, avg_num_nodes=self.avg_num_nodes
        )
...
def load_data(args):
    ...
    dataset = partial(
        MDAnalysisDataset,
        dataset_name="protein_md",
        data_dir=args.data_dir,
        cutoff_rate=0.25,
        virtual_channels=8,
        load_cached=True,
        delta_frame=15,
        backbone=args.backbone_atoms_only,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Timer for training in string format, e.g., "00:08:00:00" for 8 hours',
    )
    parser.add_argument(
    ...
    # Model architecture
    parser.add_argument(
        "--hidden_dim", type=eval, default=384, help="Hidden dimension(s)"
    )
    parser.add_argument("--layers", type=eval, default=4, help="Layers per scale")
    parser.add_argument(
        "--equivariance", type=str, default="SEn", help="Type of equivariance"
    )
    parser.add_argument(
        "--orientations",
        type=int,
        default=8,
        help="Number of orientations controlling vector outputs",
    )
    ...
    parser.add_argument(
        ...
        type=eval,
        default=[],
        help='Features to use as scalars: ["coords"]',
    )
    parser.add_argument(
        "--vector_features",
        type=eval,
        default=["velocity"],
        help='Features to use as vectors: ["coords"]',
    )
    parser.add_argument(
        "--test_rot_trans",
        type=bool,
        default=False,
        help="Rotate and translate the test set for evaluation",
    )

    # Data and logging
    parser.add_argument(
        "--data_dir", type=str, default="./datasets/protein_md/", help="Data directory"
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Use oonly backbone or all atoms of the protein",
    )

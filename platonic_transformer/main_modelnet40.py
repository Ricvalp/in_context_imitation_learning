...
class ModelNet40Model(pl.LightningModule):
    def __init__(self, args):
        ...
        ) + 3 * (  # x,y,z coordinates as scalars
            "normals" in self.hparams.scalar_features
        )  # normal components as scalars
        in_channels_vector = (
    ...
    def forward(self, data):
        ...
        if (self.training and self.hparams.train_augm) or (
            not self.training and self.hparams.test_augm
        ):
        ...
        if "pose" in self.hparams.vector_features:
            vec.append(
                rot.transpose(-2, -1).unsqueeze(0).expand(data.pos.shape[0], -1, -1)
            )
        ...
        # Forward pass
        pred, _ = self.net(
            x, data.pos, data.batch, vec=vec, avg_num_nodes=self.avg_num_nodes
        )
        return pred
...
def main(args):
    ...
    if args.log:
        ...
        logger = pl.loggers.WandbLogger(
            project="PlatonicTransformer-ModelNet40", config=args, save_dir=save_dir
        )
    else:
        logger = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=1e-12, help="Weight decay"
    )
    ...
    parser.add_argument(
        "--equivariance", type=str, default="Tn", help="Type of equivariance"
    )
    ...
    parser.add_argument(
        ...
        type=eval,
        default=["normals"],
        help='Features to use as scalars: ["coords", "normals"]',
    )
    ...
    parser.add_argument(
        ...
        type=eval,
        default=[],
        help='Features to use as vectors: ["coords", "normals", "pose"]',
    )
    ...
    parser.add_argument(
        ...
        type=eval,
        default=False,
        help="Use rotation augmentation during testing",
    )
    parser.add_argument(
    ...
    # Data and logging
    parser.add_argument(
        "--data_dir", type=str, default="./datasets/modelnet", help="Data directory"
    )

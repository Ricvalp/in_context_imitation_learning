...
from equi_icil.platonic_config import get_config
from equi_icil.utils import (
...
from equi_icil.imitation_datasets import (
    DatasetConfig,
    SparseDatasetConfig,
...
default_config_path = str(Path(__file__).resolve().parents[1] / "equi_icil" / "platonic_config.py")
...
flags.DEFINE_bool(
    "debug_dataset", False, "Load only a subset of each task for faster debugging"
)
...
flags.DEFINE_enum(
    "policy",
    None,
    ["platonic_diffusion", "in_context"],
    "Policy variant to train.",
)
...
def build_model(cfg: ConfigDict) -> PlatonicDiffusionPolicy:
    ...
    policy_type = getattr(cfg.model, "policy", "platonic_diffusion")
    if policy_type == "in_context" and cfg.dataset_mode == "sparse":
        raise ValueError("In-context policy currently supports dense datasets only.")
    ...
    if not transformer_cfg.dense_mode and use_cls_token:
        logging.warning(
            "CLS token is not supported in sparse transformer mode; disabling it."
        )
        use_cls_token = False

    if policy_type == "in_context":
        if transformer_cfg.mask_action_to_obs and not transformer_cfg.attention:
            raise ValueError(
                "In-context policy w/ masking requires transformer.attention to be enabled."
            )
        if not transformer_cfg.dense_mode:
            raise ValueError(
                "In-context policy requires transformer.dense_mode to be True."
            )
        ...
        model_cfg = InContextPlatonicPolicyConfig(
            ...
            transformer_mask_action_to_obs=transformer_cfg.mask_action_to_obs,

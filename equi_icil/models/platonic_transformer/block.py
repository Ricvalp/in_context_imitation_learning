...
class PlatonicBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            conditioning (Optional[Tensor]): Conditioning embeddings of shape (B, cond_dim).

        Returns:
            Tensor: Output feature tensor of the same shape [..., G*C].
        """
        if self.conditioning and conditioning is not None:
            cond_chunks = self.adaLN_modulation(conditioning).chunk(6, dim=-1)
            shift_msa, scale_msa, gate_msa, shift_ffn, scale_ffn, gate_ffn = [
                self._broadcast_condition(chunk, x, batch) for chunk in cond_chunks
            ]
        else:
        ...
        if self.norm_first:
            ...
            interaction_out = self._interaction_block(
                normed_x, pos, batch, mask, attn_mask, avg_num_nodes
            )
            ...
            normed_ff = self._apply_condition(normed_ff, shift_ffn, scale_ffn)
            ...
            interaction_out = self._interaction_block(
                x, pos, batch, mask, attn_mask, avg_num_nodes
            )
            if self.gamma_1 is not None:
                interaction_out = self.gamma_1 * interaction_out
            residual = self.drop_path1(interaction_out)
            ...
            x = self._apply_condition(x, shift_msa, scale_msa)

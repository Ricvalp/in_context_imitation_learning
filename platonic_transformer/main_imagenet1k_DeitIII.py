...
try:
    ...
    print(
        "timm is not installed. LAMB optimizer will not be available. Run 'pip install timm'"
    )
    Lamb = None
...
class ImageNetModel(pl.LightningModule):
    ...
    def forward(self, data):
        ...
        if self.hparams.model_type == "platoformer":
            pred, _ = self.net(
                data.x, data.pos, data.batch, vec=None, avg_num_nodes=self.avg_num_nodes
            )
        elif self.hparams.model_type == "vit":
...
def load_data(args):
    ...
    full_train_dataset = ImageNet(
        data_dir="/scratch-nvme/ml-datasets/imagenet/ILSVRC/Data/CLS-LOC/",
        split="train",
    ...
    print(
        f"Training on {len(train_dataset)} samples ({100*len(train_dataset)/len(full_train_dataset):.2f}% of full training set)"
    )
...
def main(args):
    ...
    if args.log:
        ...
        logger = pl.loggers.WandbLogger(
            project="Platonic-ImageNet", config=args, save_dir=save_dir
        )
    else:
        logger = None

    callbacks = [
        pl.callbacks.ModelCheckpoint(monitor="valid_acc", mode="max", save_last=True),
        TimerCallback(),
    ]
    if args.log:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ImageNet Point Cloud Classification Training"
    )  # Updated description
    ...
    parser.add_argument(
        "--warmup", type=int, default=20, help="Number of warmup epochs"
    )
    ...
    parser.add_argument(
        "--layers", type=eval, default=5, help="Number of layers or layers per scale"
    )
    parser.add_argument(
        "--solid_name",
        type=str,
        default="trivial_2",
        help='Type of symmetry group ("trivial_2", "cyclic_#", "dihedral_#", "flop") (Tn, SEn)',
    )
    ...
    parser.add_argument(
        "--head_dim", type=int, default=None, help="Implicitly defines number of heads"
    )
    parser.add_argument(
        "--rope_sigma", type=eval, default=1.0, help="Sigma for RFF positional encoding"
    )
    parser.add_argument(
        "--ape_sigma", type=eval, default=10.0, help="Sigma for RFF positional encoding"
    )
    ...
    parser.add_argument(
        "--learned_freqs", type=eval, default=True, help="Learnable frequencies for RFF"
    )
    parser.add_argument(
        "--dense_mode", type=eval, default=True, help="Use dense attention"
    )
    parser.add_argument(
        "--mean_aggregation",
        type=eval,
    ...
    parser.add_argument(
        "--attention", type=eval, default=True, help="Use attention in the model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Stochastic depth rate (uniform) for both models",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Side length of the square image patches",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/projects/0/prjs1161/imagenet",
        help="Data directory",
    )

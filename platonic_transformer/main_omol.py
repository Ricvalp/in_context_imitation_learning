...
class OMolModel(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if self.hparams.head_dim is not None:
            num_heads = self.hparams.hidden_dim // (self.hparams.head_dim * PLATONIC_GROUPS[solid_name].G)
    ...
    def configure_optimizers(self):
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn == 'freqs': no_decay.add(fpn)
                elif pn.endswith('bias') or ('layer_scale' in pn): no_decay.add(fpn)
                elif pn.endswith('weight') and isinstance(m, whitelist_weight_modules): decay.add(fpn)
                elif pn.endswith('kernel'): decay.add(fpn)
        ...
        if self.hparams.cosine_scheduler:
            ...
            return {"optimizer": optimizer, "lr_scheduler": scheduler, "monitor": "valid MAE (energy/atom) [meV]"}
        else:
...
def main(args):
    ...
    train_loader, val_loader, test_loader, _, _ = get_omol_loaders(
        ...
        use_charges=False, seed=args.seed, debug_subset=args.debug_subset,
        referencing=args.referencing, include_hof=args.include_hof, scale_shift=args.scale_shift,
        recalculate=args.recalculate_stats, use_k_hot=args.use_khot_encoding  
    )
    ...
    logger = pl.loggers.WandbLogger(
        project=args.wandb_project_name, name=None, config=vars(args),
    ...
    if args.timer: callbacks.append(Timer(duration=args.timer))
   
    if args.load_weights:
        model = OMolModel.load_from_checkpoint(checkpoint_path=args.load_weights, args=args)
    else:
    ...
    trainer = pl.Trainer(
        logger=logger, max_epochs=args.epochs, callbacks=callbacks,
        gradient_clip_val=1.0, accelerator=accelerator, devices=devices,
        enable_progress_bar=args.enable_progress_bar, precision=args.precision,
    ...
    if not args.test_ckpt:
        ...
        best_ckpt_path = callbacks[2].best_model_path if callbacks[2].best_model_path else "last"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='OMol Energy and Force Prediction Training')
    ...
    parser.add_argument('--lambda_F', type=float, default=5.0, help='Weight for force loss')
    
    # Model architecture
    parser.add_argument('--hidden_dim', type=int, default=1152, help='Hidden dimension of the model')
    parser.add_argument('--layers', type=int, default=14, help='Number of layers in the model')
    parser.add_argument('--drop_path_rate', type=float, default=0.0, help='Stochastic depth rate')
    parser.add_argument('--predict_forces', type=eval, default=True, help='Enable direct force prediction instead of using gradients.')

    # Platonic Transformer specific parameters
    parser.add_argument('--solid_name', type=str, default="tetrahedron", help='Group name for Platonic solids (Platoformer only)')
    parser.add_argument('--num_heads', type=int, default=72, help='Number of attention heads (Platoformer only)')
    parser.add_argument('--head_dim', type=int, default=None, help='Implicitly defines number of heads (Platoformer only)')
    parser.add_argument('--rope_sigma', type=float, default=4, help='Sigma for RoPE (Platoformer only)')
    parser.add_argument('--ape_sigma', type=eval, default=None, help='Sigma for APE (Platoformer only)')
    parser.add_argument('--freq_init', type=str, default='spiral', choices=['random', 'spiral'], help='Frequency init for RoPE (Platoformer only)')
    parser.add_argument('--use_key', type=eval, default=False, help='Use key projection with RoPE (Platoformer only)')
    parser.add_argument('--dropout', type=float, default=0.0, help='Dropout rate (Platoformer only)')
    parser.add_argument('--norm_first', type=eval, default=True, help='Use LayerNorm before attention (Platoformer only)')
    parser.add_argument('--learned_freqs', type=eval, default=True, help='Learn frequencies for RoPE (Platoformer only)')
    parser.add_argument('--dense_mode', type=eval, default=False, help='Enable dense attention blocks (Platoformer only)')
    parser.add_argument('--mean_aggregation', type=eval, default=False, help='Use mean aggregation instead of sum (Platoformer only)')
    parser.add_argument('--attention', type=eval, default=True, help='Use attention in PlatonicConv (Platoformer only)')
    parser.add_argument('--ffn_readout', type=eval, default=True, help='Feed-forward readout (Platoformer only)')

    parser.add_argument('--layer_scale', type=eval, default=None, help='Layer scaling factor for PlatonicTransformer')
    
    # Training features
    parser.add_argument('--train_augm', type=eval, default=True, help='Use rotation augmentation')
    
    # Input features
    parser.add_argument('--scalar_features', type=eval, default=[], help='Additional scalar features, e.g., ["coords", "charges"]')
    parser.add_argument('--vector_features', type=eval, default=[], help='Additional vector features, e.g., ["coords"]')
    ...
    parser.add_argument('--data_dir', type=str, default='/opt/datasets/ivi/ivi_omol', help='Data directory')
    ...
    parser.add_argument('--include_hof', type=eval, default=False, help='Normalize target using HOF values')
    parser.add_argument('--scale_shift', type=eval, default=False, help='Use scale and shift normalization')
    parser.add_argument('--use_khot_encoding', type=eval, default=True, help='Use k-hot encoding for atom types')
    ...
    parser.add_argument('--precision', type=str, default='32', choices=['16-mixed', 'bf16-mixed', '32'], help='Training precision')
    ...
    parser.add_argument('--load_weights', type=str, default=None, help='Path to load model weights from for a new run')

"""Entry point for training the Transformer-based in-context diffusion policy."""
...
from equi_icil.in_context_diffusion_config import get_config
...
default_config_path = str(
    ...
    / "equi_icil"
    / "in_context_diffusion_config.py"
)
...
def _apply_overrides(cfg: ConfigDict) -> ConfigDict:
    ...
    cfg.model.transformer.mask_action_to_obs = bool(
        cfg.model.transformer.mask_action_to_obs
    )
    return cfg
...
def build_dataloaders(cfg: ConfigDict):
    ...
    if getattr(cfg, "dataset_mode", "dense") != "dense":
...
def build_model(cfg: ConfigDict) -> InContextDiffusionPolicy:
    """Instantiate the Transformer-based in-context diffusion policy from ``cfg``."""
    if getattr(cfg.model, "policy", "in_context_diffusion") != "in_context_diffusion":
        raise ValueError(
            "Config mismatch: expected model.policy == 'in_context_diffusion'."
        )
    ...
    model_cfg = InContextDiffusionPolicyConfig(
        ...
        transformer_layer_norm_eps=transformer_cfg.layer_norm_eps,
...
def evaluate(
    model: InContextDiffusionPolicy,

...
class PlatonicConv(nn.Module):
    ...
    def __init__(
        ...
        if embed_dim % (num_heads // self.num_G) != 0:
            raise ValueError(
                f"embed_dim ({embed_dim}) must be divisible by (num_heads // num_group) = "
    ...
    def _forward_shared(self, x: Tensor, pos: Tensor):
        ...
        k_raw = (
            self.k_proj(x)
            if ((self.rope_emb is None) or self.use_key)
            else torch.ones_like(q_raw)
        )

        # Reshape for multi-head processing: [..., G * H * D_h] -> [..., G, H, D_h]
        q = q_raw.view(
            *leading_dims, self.num_G, self.effective_num_heads, self.head_dim
        )
        v = v_raw.view(
            *leading_dims, self.num_G, self.effective_num_heads, self.head_dim
        )
    ...
    def graph_scattered_attention(
        ...
        a = scatter_softmax(
            scores.flatten(), group_ids.flatten(), dim=0, dim_size=N * GH
        ).view(E, GH)

        weighted = (a.unsqueeze(-1) * v_dst).reshape(-1, D)  # [E*GH, D]

        out = scatter_sum(weighted, group_ids.flatten(), dim=0, dim_size=N * GH).view(
            N, GH, D
        )
    ...
    def _forward_dense(
        ...
        if self.attention:
            ...
            q_sdpa = q_rope.view(
                B, S, self.num_G * self.effective_num_heads, self.head_dim
            ).transpose(1, 2)
            k_sdpa = k_rope.view(
                B, S, self.num_G * self.effective_num_heads, self.head_dim
            ).transpose(1, 2)
            ...
            if mask is not None:
                if mask.dim() != 2:
                    raise ValueError("Key padding mask must have shape (B, N).")
                key_pad = ~mask
                combined_mask = (
                    key_pad[:, None, None, :].expand(-1, q_sdpa.shape[1], -1, -1)
                )
            if attn_mask is not None:
                ...
                if pairwise.shape[1] == 1:
                    pairwise = pairwise.expand(-1, q_sdpa.shape[1], -1, -1)
                if combined_mask is None:
                    combined_mask = pairwise
                else:
                    combined_mask = combined_mask.logical_or(pairwise)

            attn_mask_tensor = combined_mask
            with sdpa_kernel(
                [
                ...
                attn_output = F.scaled_dot_product_attention(
                    q_sdpa, k_sdpa, v_sdpa, attn_mask=attn_mask_tensor
                )
            ...
            if attn_mask is not None:
                raise NotImplementedError(
                    "Pairwise attention masks require attention=True in PlatonicConv."
                )
            if mask is not None:
    ...
    def forward(
        ...
        if is_graph_mode:
            ...
            return self._forward_dense(
                x, pos, mask, avg_num_nodes=avg_num_nodes, attn_mask=attn_mask
            )

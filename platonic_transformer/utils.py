...
def format_batch_for_esen(batch):
    ...
    data_dict = AtomicDataObj(
        ...
        charge=(
            torch.zeros(len(natoms), device=batch.pos.device)
            if not hasattr(batch, "charge")
            else batch.charge
        ),
        spin=(
            torch.zeros(len(natoms), device=batch.pos.device)
            if not hasattr(batch, "spin")
            else batch.spin
        ),
    ...
    if hasattr(batch, "cell"):
        ...
        data_dict.cell = (
            torch.eye(3, device=batch.pos.device).unsqueeze(0).repeat(len(natoms), 1, 1)
            * 20.0
        )
    ...
    if hasattr(batch, "pbc"):
        ...
        data_dict.pbc = torch.zeros(
            len(natoms), 3, dtype=torch.bool, device=batch.pos.device
        )
...
class RandomJitter(BaseTransform):
    ...
    def __call__(self, data):
        noise = torch.clamp(
            self.sigma * torch.randn_like(data.pos), min=-self.clip, max=self.clip
        )
...
class RandomRotatePerturbation(BaseTransform):
    ...
    def __call__(self, data):
        angles = torch.clamp(
            self.angle_sigma * torch.randn(3), min=-self.angle_clip, max=self.angle_clip
        ).to(data.pos.device)
        ...
        Rx = torch.tensor(
            [[1, 0, 0], [0, cos_x, -sin_x], [0, sin_x, cos_x]], device=data.pos.device
        )

        Ry = torch.tensor(
            [[cos_y, 0, sin_y], [0, 1, 0], [-sin_y, 0, cos_y]], device=data.pos.device
        )

        Rz = torch.tensor(
            [[cos_z, -sin_z, 0], [sin_z, cos_z, 0], [0, 0, 1]], device=data.pos.device
        )
...
class TimerCallback(pl.Callback):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        trainer.logger.experiment.log(
            {"Total Training Time (min)": total_training_time}
        )
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        trainer.logger.experiment.log(
            {"Test Inference Time (min)": self.test_inference_time}
        )
...
class StopOnPersistentDivergence(pl.Callback):
    ...
    def __init__(
        ...
        if not isinstance(patience, int) or patience < 1:  # Patience must be at least 1
            raise ValueError(
                "Argument `patience` must be an integer greater than or equal to 1."
            )
    ...
    def on_validation_epoch_end(
        ...
        if current_epoch < self.grace_epochs:
            if (
                self.verbose and current_epoch == 0
            ):  # Print only once at the start of training
                print(
        ...
        if current_metric_value is None:
            if (
                self.verbose and current_epoch == self.grace_epochs
            ):  # Print warning once when check becomes active
                print(
        ...
        if current_metric_value > self.threshold:
            ...
            if (
                self.verbose and self.consecutive_exceeds_count > 0
            ):  # Log reset only if it was counting
                print(
                    f"{status_message} Metric NOT ABOVE threshold. Resetting consecutive count."
                )
...
class TrainingTimerCallback(pl.Callback):
    ...
    def on_train_batch_end(
        ...
        if self.measuring and self.in_training and self.batch_start_time is not None:
            ...
            full_training_time = (
                batch_end_time - self.batch_start_time
            ) * self.multiplier
    ...
    def _register_hooks(self, pl_module):
        ...
        def hook_fn(module, input, output, name):
            ...
            if isinstance(output, torch.Tensor):
                ...
                has_nan = any(
                    torch.isnan(x).any() for x in output if isinstance(x, torch.Tensor)
                )
            else:
                has_nan = False

...
class CIFAR10DiffusionModel(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.register_buffer('patch_pos', build_patch_grid(h.patch_size), persistent=False)
    ...
    def build_conditioning(self, timesteps: torch.Tensor, labels: Optional[torch.Tensor], drop_prob: float) -> torch.Tensor:
        ...
        if self.class_embed is not None and labels is not None:
            ...
            if drop_prob > 0.0:
                drop_mask = (torch.rand_like(timesteps.float()) < drop_prob).float().unsqueeze(-1)
                class_cond = class_cond * (1.0 - drop_mask)
            cond = cond + class_cond
        return cond

    def predict_noise(self, x: torch.Tensor, pos: torch.Tensor, batch_idx: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        pred, _ = self.net(x, pos, batch_idx, conditioning=cond, avg_num_nodes=float(self.num_patches))
        return pred
    ...
    def training_step(self, batch: Batch, batch_idx: int) -> torch.Tensor:
        ...
        timesteps = torch.randint(0, self.hparams.num_steps, (batch_size,), device=x0.device, dtype=torch.long)

        sqrt_alpha = self.sqrt_alphas_cumprod[timesteps][batch.batch].unsqueeze(-1)
        sqrt_one_minus = self.sqrt_one_minus_alphas_cumprod[timesteps][batch.batch].unsqueeze(-1)
    ...
    @torch.no_grad()
    def sample(self, num_samples: int, guidance_scale: float = 0.0, class_labels: Optional[torch.Tensor] = None, use_ema: bool = True) -> torch.Tensor:
        device = self.device
        ...
        for idx in range(num_samples):
            ...
            if class_labels is not None:
                label_tensor = torch.tensor(int(class_labels[idx].item()), dtype=torch.long)
        ...
        for t in reversed(range(self.hparams.num_steps)):
            ...
            labels = batch_template.y if (class_labels is not None and hasattr(batch_template, 'y')) else None
            cond = self.build_conditioning(t_tensor, labels, 0.0)
            cond = cond.to(device)
            noise_pred = self.predict_noise(x, batch_template.pos, batch_template.batch, cond)
            ...
            if guidance_scale > 0.0 and self.class_embed is not None:
                ...
                noise_uncond = self.predict_noise(x, batch_template.pos, batch_template.batch, uncond)
                noise_pred = noise_uncond + guidance_scale * (noise_pred - noise_uncond)

            betas_t = self.betas[t_tensor][batch_template.batch].unsqueeze(-1)
            sqrt_one_minus = self.sqrt_one_minus_alphas_cumprod[t_tensor][batch_template.batch].unsqueeze(-1)
            sqrt_recip_alpha = torch.sqrt(1.0 / (1.0 - betas_t))
            model_mean = sqrt_recip_alpha * (x - betas_t / sqrt_one_minus * noise_pred)

            if t > 0:
                noise = torch.randn_like(x)
                posterior_var = self.posterior_variance[t_tensor][batch_template.batch].unsqueeze(-1)
...
def load_data(args):
    ...
    full_train_dataset = torchvision.datasets.CIFAR10(root=args.data_dir, train=True, transform=transform, download=True)
    ...
    def collate_fn(batch):
        ...
        for image_tensor, label in batch:
            ...
            data = Data(x=patches, pos=patch_pos.clone(), y=torch.tensor(label, dtype=torch.long))
...
def main(args):
    ...
    logger = pl.loggers.WandbLogger(project="Platonic-CIFAR10-Diffusion", save_dir=os.path.join(os.getcwd(), "logs")) if args.log else None
    callbacks = [pl.callbacks.ModelCheckpoint(monitor='val_loss', mode='min', save_last=True), TimerCallback()]
    if args.log:
    ...
    if not args.sample_only:
        ...
        model = CIFAR10DiffusionModel.load_from_checkpoint(args.sample_ckpt, map_location=device)
        model = model.to(device)
        samples = model.sample(args.sample_count, guidance_scale=args.guidance_scale, use_ema=args.use_ema)
        out_dir = os.path.join(os.getcwd(), "samples")
        os.makedirs(out_dir, exist_ok=True)
        for idx, img in enumerate(samples):
            img = (img.clamp(-1, 1) + 1.0) / 2.0
            torchvision.utils.save_image(img.cpu(), os.path.join(out_dir, f"sample_{idx:04d}.png"))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CIFAR10 Diffusion Training with Platonic Transformer')
    ...
    parser.add_argument('--conditioning_dim', type=int, default=512, help='Conditioning embedding dimension')
    parser.add_argument('--conditioning_mlp_dim', type=int, default=2048, help='Hidden size of conditioning MLP')
    parser.add_argument('--time_embed_dim', type=int, default=2048, help='Hidden dimension for timestep MLP')
    parser.add_argument('--class_cond', type=eval, default=True, help='Enable class conditional guidance')
    parser.add_argument('--cfg_drop_prob', type=float, default=0.1, help='Classifier-free guidance drop probability during training')
    parser.add_argument('--guidance_scale', type=float, default=3.0, help='Classifier-free guidance scale at sampling time')
    parser.add_argument('--use_ema', type=eval, default=True, help='Track EMA weights for sampling')
    parser.add_argument('--ema_decay', type=float, default=0.9999, help='EMA decay factor')
    parser.add_argument('--patch_size', type=int, default=4, help='Patch size for tokenization')
    parser.add_argument('--hidden_dim', type=int, default=768, help='Transformer hidden dimension')
    parser.add_argument('--layers', type=int, default=12, help='Number of transformer layers')
    parser.add_argument('--num_heads', type=int, default=12, help='Number of attention heads')
    parser.add_argument('--ffn_dim_factor', type=int, default=4, help='FFN dimensional multiplier')
    parser.add_argument('--solid_name', type=str, default="cyclic_4", help='Symmetry group name')
    parser.add_argument('--dense_mode', type=eval, default=True, help='Operate in dense mode')
    parser.add_argument('--ffn_readout', type=eval, default=True, help='Use FFN readout heads')
    parser.add_argument('--mean_aggregation', type=eval, default=False, help='Use mean aggregation inside pooling')
    parser.add_argument('--dropout', type=float, default=0.0, help='Dropout rate')
    parser.add_argument('--norm_first', type=eval, default=True, help='Pre-norm transformer blocks')
    parser.add_argument('--drop_path_rate', type=float, default=0.1, help='Drop path rate')
    parser.add_argument('--layer_scale_init_value', type=float, default=1e-5, help='Layer scale init')
    parser.add_argument('--attention', type=eval, default=True, help='Enable attention mode in PlatonicConv')
    ...
    parser.add_argument('--learned_freqs', type=eval, default=True, help='Learnable RoPE frequencies')
    parser.add_argument('--freq_init', type=str, default='spiral', choices=['random', 'spiral'], help='RoPE init')
    parser.add_argument('--use_key', type=eval, default=False, help='Use key projection under RoPE')
    parser.add_argument('--data_dir', type=str, default="./datasets/cifar10", help='Data directory')
    parser.add_argument('--val_split', type=float, default=0.05, help='Validation split from training set')
    ...
    parser.add_argument('--sample_only', type=eval, default=False, help='Skip training and only sample')
    parser.add_argument('--sample_ckpt', type=str, default=None, help='Checkpoint path for sampling')
    parser.add_argument('--sample_count', type=int, default=16, help='Number of samples to draw')

...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, pos, edge_index, batch, sigma, rot=None):
        ...
        if (
            c_noise.shape[0] == batch.shape[0]
        ):  # if c_noise is of shape [num_nodes, 1], then we need to convert it to [num_graphs, 1]
            c_noise = torch.unique_consecutive(c_noise).unsqueeze(-1)
        elif (
            c_noise.numel() == 1
        ):  # if just a scalar is provided, convert to [num_graphs, 1]
        ...
        scalars_out, vecs_out = self.model(
            ...
            batch,
            vec=None,
            time_conditioning=c_noise,
            avg_num_nodes=18.0,
        )  # hard-coded avg num nodes for QM9!
...
class QM9Model(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.validation_frequency = getattr(
        ...
        self.model = EDMPrecond(
            self.net, sigma_data=self.hparams.sigma_data, hparams=self.hparams
        )
        self.criterion = EDMLoss(
            sigma_data=self.hparams.sigma_data, hparams=self.hparams
        )
    ...
    def _evaluate_batch_metrics(self, molecules):
        ...
        return {
            "atom_stability": 100.0 * count_atm_stable / count_atm_total,
            "molecule_stability": 100.0 * count_mol_stable / count_mol_total,
        }
    ...
    def on_test_epoch_end(self):
        results = self.validate(
            num_molecules=10000, batch_size=self.hparams.batch_size, rdkit_metrics=True
        )
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.cosine_scheduler:
            scheduler = CosineAnnealingLR(
                optimizer, T_max=self.trainer.max_epochs, eta_min=self.hparams.lr / 100
            )
    ...
    def sample(self, num_molecules=100):
        ...
        with torch.no_grad():
            ...
            x_0 = torch.randn(
                [len(batch_idx), 5 + 1 * self.hparams.use_charges], device=self.device
            )
        ...
        for i in range(batch_idx.max() + 1):
            ...
            if self.hparams.use_charges:
                ...
                charges = (
                    (
                        samples[0][batch_idx == i, -1]
                        * self.hparams.normalize_charge_factor
                    )
                    .round()
                    .long()
                )
    ...
    def validate(self, num_molecules=10000, batch_size=100, rdkit_metrics=True):
        ...
        if rdkit_metrics:
            [validity, uniqueness, novelty], _ = self.molecule_analyzer.evaluate(
                molecules
            )
            results_dict.update(
                {
...
def load_data(args):
    ...
    train_set = QM9Dataset(
        split="train", root=args.data_dir, use_charges=args.use_charges
    )
    val_set = QM9Dataset(split="val", root=args.data_dir, use_charges=args.use_charges)
    ...
    if args.dataset_fraction < 1.0:
        ...
        train_set = torch.utils.data.Subset(
            train_set, torch.randperm(len(train_set))[:train_subset_len]
        )
...
def main(args, checkpoint_interval=1, check_val_every_n_epoch=1):
    ...
    # Load data
    train_loader, val_loader, num_atoms_sampler, smiles_list, dataset_info = load_data(
        args
    )
    ...
    if args.log:
        ...
        logger = pl.loggers.WandbLogger(
            project="Platonic-QM9-Gen", name=None, config=args, save_dir=save_dir
        )
    else:
        logger = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--warmup", type=int, default=10, help="Number of warmup epochs for scheduler."
    )
    ...
    # --- Model Architecture ---
    parser.add_argument(
        "--hidden_dim", type=int, default=768, help="Hidden dimension of the model."
    )
    parser.add_argument(
        "--layers", type=int, default=12, help="Number of layers in the model."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="tetrahedron",
        help="Platonic solid group name (e.g., tetrahedron, octahedron).",
    )
    parser.add_argument(
        "--num_heads", type=int, default=24, help="Number of attention heads."
    )
    ...
    parser.add_argument(
        ...
        help="Frequency init for RoPE.",
    )
    parser.add_argument(
        "--use_key", type=eval, default=False, help="Use key projection with RoPE."
    )
    ...
    parser.add_argument(
        "--learned_freqs", type=eval, default=True, help="Learn frequencies for RoPE."
    )
    parser.add_argument(
        "--dense_mode", type=eval, default=False, help="Enable dense attention blocks."
    )
    parser.add_argument(
        "--mean_aggregation",
        type=eval,
    ...
    parser.add_argument(
        "--attention", type=eval, default=True, help="Use attention in PlatonicConv."
    )
    parser.add_argument(
        "--ffn_readout", type=eval, default=True, help="Use a feed-forward readout."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=eval,
        default=True,
        help="Use time conditioning in the model.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="Stochasticity parameter for EDM sampling.",
    )
    parser.add_argument(
        "--sigma_max", type=float, default=1.0, help="Maximum noise sigma for EDM."
    )
    parser.add_argument(
        "--num_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Assumed standard deviation of the data distribution.",
    )
    parser.add_argument(
        "--normalize_x_factor",
        type=float,
        default=4.0,
        help="Normalization factor for atom features.",
    )
    parser.add_argument(
        "--normalize_charge_factor",
        type=float,
        default=8.0,
        help="Normalization factor for atomic charges.",
    )
    ...
    parser.add_argument(
        ...
        type=eval,
        default=True,
        help="Include atomic charges as a feature.",
    )
    parser.add_argument(
        "--train_augm",
        type=eval,
        default=True,
        help="Use SO(3) rotation augmentation during training.",
    )
    parser.add_argument(
        "--validation_frequency",
        type=int,
        default=20,
        help="Frequency (in epochs) for running full validation.",
    )
    parser.add_argument(
        "--data_dir", type=str, default="./datasets/qm9", help="Data directory for QM9."
    )
    parser.add_argument(
        "--dataset_fraction",
        type=float,
